...
import { createLiveStreamLane, type LiveStreamLane } from './live-stream-lane.js';
...
function createGmeetBotPipeline(
  ...
  return {
    ...
    async stop() { await lane.dispose(); if (live) await live.dispose(); },
    feedAudio: (channel, glowName, pcm, tsMs) => {
      lane.feedAudio(channel, glowName, pcm, tsMs);
      // B-full: additively tap the SAME frame to the live streaming lane (never throws).
      live?.feed(channel, glowName, pcm);
    },
...
function createMixedBotPipeline(
  ...
  return {
    ...
    async stop() { if (transcriber) await transcriber.dispose(); if (live) await live.dispose(); },
    feedAudio() { /* not the mixed lane (mixed has no per-channel glow) */ },
    feedMixedAudio: (pcm, tsMs) => {
      transcriber?.feedAudio(pcm, tsMs);
      live?.feed(0, undefined, pcm);   // B-full tap (mixed = single channel 0)
    },
...
export function createBotPipeline(
  ...
  // B-full live streaming lane (additive; null when not configured / unsupported).
  const live = createLiveStreamLane(inv);
  if (isMixedLanePlatform(inv.platform)) {
    return createMixedBotPipeline(
      transcribe, sink, hintKindForPlatform(inv.platform),
      inv.language ?? undefined, opts.onError, opts.createMixedTranscriber, live,
    );
  }
  return createGmeetBotPipeline(transcribe, sink, opts.config, opts.onError, live);
}
