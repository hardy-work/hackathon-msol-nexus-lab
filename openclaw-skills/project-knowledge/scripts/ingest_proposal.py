...
``.runtime/ingest-proposals`` or ``PROJECT_KNOWLEDGE_STATE_DIR``.
"""
...
SCHEMA = "nexus-project-knowledge/ingest-proposal/v2"
...
def authorize_requester(actor: str, roles: str | list[str] | None = None,
    ...
    return False, "Slack user không nằm trong allowlist project_knowledge:ingest"
...
def _check_source(source: Path, root: Path, confirmed_doc_id: str | None = None
    ...
    max_bytes = int(os.getenv("PROJECT_KNOWLEDGE_INGEST_MAX_BYTES", str(100 * 1024 * 1024)))
    if size > max_bytes:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Slack-triggered Project Knowledge ingest proposal")
