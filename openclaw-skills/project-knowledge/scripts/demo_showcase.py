#!/usr/bin/env python3
"""Curated, one-process Nexus Project Knowledge showcase.
...
def configure_demo_environment() -> None:
    ...
    os.environ.setdefault("PROJECT_KNOWLEDGE_ACTOR", "local-demo")
    os.environ.setdefault("PROJECT_KNOWLEDGE_ROLES", "project_member")
    os.environ.setdefault("PROJECT_KNOWLEDGE_DEMO_MODE", "1")
    os.environ.setdefault("PROJECT_KNOWLEDGE_STATE_DIR", str(ROOT / ".runtime" / "demo"))
    os.environ.setdefault(
        "PROJECT_KNOWLEDGE_COVERAGE_GRANTS",
        json.dumps({"Đô": ["project_knowledge:approve_coverage"]}, ensure_ascii=False),
    )
    os.environ.setdefault(
...
SCENARIOS = (
    ...
    Scenario(
        ...
        "not_in_kb",
        "Project Knowledge từ chối ghi và chỉ tạo proposal cần approval.",
...
def run_showcase(*, use_cache: bool = True) -> dict[str, Any]:
    ...
    for index, scenario in enumerate(SCENARIOS, start=1):
        ...
        payload = runtime.query(
            "nexus",
            scenario.question,
            actor=os.environ["PROJECT_KNOWLEDGE_ACTOR"],
            roles=os.environ["PROJECT_KNOWLEDGE_ROLES"],
...
def print_human(report: dict[str, Any]) -> None:
    ...
    print("NEXUS · PROJECT KNOWLEDGE SHOWCASE")
    print(
