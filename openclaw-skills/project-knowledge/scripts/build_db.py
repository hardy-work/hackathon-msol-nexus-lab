...
def main():
    ...
    con.execute("""CREATE TABLE person(
        ...
        src_task VARCHAR, src_actual VARCHAR)""")
    for p in sorted((ROOT / "wiki" / "entities").glob("*.md")):
        ...
        con.execute("INSERT INTO person VALUES (?,?,?,?,?,?,?,?,?,?)", [
            ...
            n["task_count"]["src"], n["actual_h"]["src"]])
    ...
    con.execute("""CREATE TABLE person_sprint(
        ...
        estimate_h DOUBLE, actual_h DOUBLE, src VARCHAR)""")
    ...
    # signed=false => bậc 1 KHÔNG được nói "chắc chắn không", chỉ được "không tìm thấy".
    con.execute("""CREATE TABLE coverage(
        entity VARCHAR, relation VARCHAR, complete_as_of VARCHAR,
        source VARCHAR, asserted_by VARCHAR, signed BOOLEAN, note VARCHAR)""")
    for c in cov:
        by = str(c.get("asserted_by") or "")
        signed = bool(c.get("signed_date")) and not by.startswith("TODO")
        con.execute("INSERT INTO coverage VALUES (?,?,?,?,?,?,?)", [
            c["entity"], c["relation"], c["complete_as_of"], c["source"],
            by, signed, c.get("note", "")])

    for t in ("dim_value", "person", "person_sprint", "doc_cell", "project_metric", "coverage"):
        n = con.execute(f"SELECT count(*) FROM {t}").fetchone()[0]
        print(f"  {t:16s} {n:4d} dòng")
    unsigned = con.execute("SELECT relation FROM coverage WHERE NOT signed").fetchall()
    con.close()
    if unsigned:
        print(f"\n⚠ coverage CHƯA KÝ: {[r[0] for r in unsigned]}"
              f"\n  -> bậc 1 sẽ hạ mọi 'chắc chắn không' của các quan hệ này xuống 'không biết'.")
    print(f"\n✓ {DB.relative_to(ROOT)}")
