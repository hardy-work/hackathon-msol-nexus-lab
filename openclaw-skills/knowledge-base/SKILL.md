...
name: knowledge-base
description: >
  Tra cứu thông tin từ Excel/wiki/facts có provenance, citation và độ tin cậy
  rõ ràng. Dùng khi người dùng hỏi về task, assignee, role, sprint, effort,
  lịch, resource, risk, issue, backlog hoặc cần onboarding theo dữ liệu dự án.
  Đây là skill truy vấn read-only; khi người dùng muốn nạp/nhập/upload/ingest,
  thêm, bổ sung, cập nhật, sửa hoặc thay thế dữ liệu/tài liệu trong LLM Wiki/KB,
  dùng skill knowledge-ingest.
---
...
Trả lời câu hỏi về dự án từ kho đã được kiểm chứng. Ưu tiên câu trả lời tất định từ DuckDB/facts; mọi số liệu phải truy được về nguồn. Không suy đoán khi kho thiếu dữ liệu.

## Cách gọi demo

Từ thư mục skill (`openclaw-skills/knowledge-base`):
...
bật route LLM bằng `KNOWLEDGE_BASE_LLM=1`; `--llm` và `--no-llm` có quyền ưu tiên
...
```bash
KNOWLEDGE_BASE_LLM=1 bash demo/run_demo.sh
```
...
`KNOWLEDGE_BASE_EMBEDDING_BACKEND=hash` để không tải model, nhưng vẫn phải tạo
...
`.runtime/` hoặc volume do `KNOWLEDGE_BASE_STATE_DIR` chỉ định.
...
Identity/version và toàn bộ upload lifecycle thuộc skill
`knowledge-ingest`. Skill truy vấn chỉ dùng các artifact đã được ingest,
review và publish; không tự nhận file hoặc khởi động pipeline ingest.
...
`KNOWLEDGE_BASE_LLM=1` nếu muốn câu hỏi mở đi qua Haiku router rồi Sonnet.

## Ranh giới với ingest

Luồng nạp tài liệu là action write-like độc lập và thuộc skill
`knowledge-ingest`. Skill này chỉ đọc corpus và trả lời với facts,
confidence, citation và freshness; không tạo proposal, không chạy extractor,
không publish và không reload runtime.
