#!/usr/bin/env bash
# One-command, offline-safe showcase for the Knowledge Base skill.
set -euo pipefail
...
export KNOWLEDGE_BASE_STATE_DIR="${KNOWLEDGE_BASE_STATE_DIR:-$ROOT/.runtime/demo}"
...
export KNOWLEDGE_BASE_APPROVAL_IDS='nexus-demo-person-role-20260803,nexus-demo-person-task-20260803'
...
if [ "$needs_build" = "1" ]; then
  ...
  HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 \
    KNOWLEDGE_BASE_EMBEDDING_BACKEND=hash \
...
if [[ "${KNOWLEDGE_BASE_LLM:-0}" =~ ^(1|true|yes|on)$ ]]; then
  echo "[demo] kiểm tra live Haiku router + Sonnet answer"
  "$PY" scripts/llm_routing_selftest.py
else
  echo "[demo] LLM routing check bỏ qua; đặt KNOWLEDGE_BASE_LLM=1 để chạy live"
fi
