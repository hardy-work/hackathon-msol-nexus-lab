...
def main() -> int:
    failures = []
    env = os.environ.copy()
    env.setdefault("KNOWLEDGE_BASE_ACTOR", "local-demo")
    env.setdefault("KNOWLEDGE_BASE_ROLES", "project_member")
    env.setdefault("KNOWLEDGE_BASE_DEMO_MODE", "1")
    env.setdefault(
    ...
    if failures:
        ...
        return 1
    print(f"✓ knowledge-base smoke test: {len(CASES)}/{len(CASES)} qua")
    return 0
