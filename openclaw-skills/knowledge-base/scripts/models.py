...
def _find_claude():
    configured = os.getenv("KNOWLEDGE_BASE_CLAUDE_BIN", "").strip()
    if configured:
        candidate = Path(configured).expanduser()
        if candidate.exists() and candidate.is_file():
            return str(candidate.resolve())
        raise FileNotFoundError(
            "KNOWLEDGE_BASE_CLAUDE_BIN không tồn tại hoặc không phải file: "
            f"{candidate}"
        )
