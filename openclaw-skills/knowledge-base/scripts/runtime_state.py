...
def state_dir(root: Path = ROOT) -> Path:
    ...
    path = Path(os.getenv("KNOWLEDGE_BASE_STATE_DIR", str(default))).expanduser().resolve()
    ...
    if path == Path(root).resolve() or any(path == item or item in path.parents
        ...
        raise ValueError(f"KNOWLEDGE_BASE_STATE_DIR nằm trong corpus read-only: {path}")
