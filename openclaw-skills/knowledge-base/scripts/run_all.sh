...
PY=""
if [ -n "${KNOWLEDGE_BASE_PYTHON:-}" ]; then
  c="$KNOWLEDGE_BASE_PYTHON"
  if command -v "$c" >/dev/null 2>&1 && "$c" -c 'import sys, openpyxl' >/dev/null 2>&1; then
    PY="$c"
  else
    echo "✗ KNOWLEDGE_BASE_PYTHON không chạy được hoặc thiếu openpyxl: $c" >&2
    exit 1
  fi
else
...
export KNOWLEDGE_BASE_ACTOR="${KNOWLEDGE_BASE_ACTOR:-local-demo}"
export KNOWLEDGE_BASE_ROLES="${KNOWLEDGE_BASE_ROLES:-project_member}"
export KNOWLEDGE_BASE_DEMO_MODE="${KNOWLEDGE_BASE_DEMO_MODE:-1}"
...
export KNOWLEDGE_BASE_STATE_DIR="$PWD/.runtime/test-runtime"
if [ -z "${KNOWLEDGE_BASE_COVERAGE_GRANTS:-}" ]; then
  export KNOWLEDGE_BASE_COVERAGE_GRANTS='{"Đô":["knowledge_base:approve_coverage"]}'
fi
export KNOWLEDGE_BASE_APPROVAL_IDS="${KNOWLEDGE_BASE_APPROVAL_IDS:-nexus-demo-person-role-20260803,nexus-demo-person-task-20260803}"
...
if [ "${KNOWLEDGE_BASE_SKIP_VECTOR:-0}" = "1" ]; then
  echo "✗ KNOWLEDGE_BASE_SKIP_VECTOR đã bị loại bỏ; muốn chạy offline hãy dùng " \
       "KNOWLEDGE_BASE_EMBEDDING_BACKEND=hash" >&2
  exit 2
fi
...
echo; echo "══ SKILL ROUTING CONTRACT (offline) ══"; "$PY" ../knowledge-ingest/scripts/routing_selftest.py
...
# Gate 3b (LLM soát nội dung) — full rebuild vẫn OPT-IN vì tốn K lệnh `claude -p`
# mỗi trang và không tất định. K=3 mặc định chạy song song; đặt
# KNOWLEDGE_BASE_REVIEW_WORKERS=1 nếu host/rate limit chỉ cho chạy tuần tự. Luồng
# ingest worktree (`ingest_flow.py --run`) mặc định bắt buộc Gate 3b; chỉ
# fixture/offline mới dùng --no-review.
