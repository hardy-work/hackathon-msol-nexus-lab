#!/usr/bin/env python3
"""Cheap query router for the Knowledge Base retrieval pipeline.
...
ROUTER_PROMPT = """Bạn là bộ định tuyến rẻ cho Knowledge Base Nexus.
...
def classify(query: str, timeout: int | None = None) -> Decision:
    ...
    if timeout is None:
        timeout = max(1, int(os.getenv("KNOWLEDGE_BASE_ROUTER_TIMEOUT_SECONDS", "60")))
    try:
        proc = subprocess.run(
