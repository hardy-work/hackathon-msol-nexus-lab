...
· nguồn bị CẮT thì trả UNVERIFIABLE, KHÔNG kết luận trang sai.
  ...
  KNOWLEDGE_BASE_REVIEW_WORKERS=1 python3 scripts/review.py --page ...  # fallback tuần tự
"""
import argparse
import json
import os
...
def review_workers(k, requested=None):
    """Return a bounded worker count, with a serial fallback for constrained hosts."""
    if k < 1:
        raise ValueError("--k phải >= 1")
    if requested is None:
        configured = os.environ.get("KNOWLEDGE_BASE_REVIEW_WORKERS", "").strip()
        if configured:
            try:
                requested = int(configured)
            except ValueError as exc:
                raise ValueError("KNOWLEDGE_BASE_REVIEW_WORKERS phải là số nguyên") from exc
        else:
            requested = DEFAULT_WORKERS
    if requested < 1:
        raise ValueError("--workers phải >= 1")
    return min(k, requested)


def run_independent(prompt, k=DEFAULT_K, workers=None):
    """Run K isolated Claude subprocesses concurrently and preserve result order."""
    worker_count = review_workers(k, workers)
    with ThreadPoolExecutor(max_workers=worker_count,
                            thread_name_prefix="gate3b-review") as pool:
        futures = [pool.submit(run_once, prompt) for _ in range(k)]
...
def review(path, k=DEFAULT_K, workers=None):
    ...
    runs = run_independent(prompt, k=k, workers=workers)
    ...
    errors = [r for r in runs if r["verdict"] == "ERR"]
    if not parsed:
        verdict = "KHÔNG CHẮC"
    elif chac:
        verdict = "FINDING"
    elif errors:
        # PASS means every independent review completed. Parallel execution can
        # surface rate-limit/timeout failures sooner, but must never lower the
        # evidence threshold by silently ignoring those failed votes.
        verdict = "KHÔNG CHẮC"
    elif unverifiable:
    ...
    if errors:
        lines.append(f"  (lượt lỗi: {[r['err'] for r in errors]})")
    if unverifiable:
...
def main():
    ...
    ap.add_argument(
        "--workers", type=int,
        help=("số phiên Claude chạy song song; mặc định min(K, 3), hoặc "
              "KNOWLEDGE_BASE_REVIEW_WORKERS"))
    a = ap.parse_args()
    try:
        workers = review_workers(a.k, a.workers)
    except ValueError as exc:
        ap.error(str(exc))
    ...
    print(
        ...
        f"({workers} song song, claude -p, model={models.REVIEW}, không công cụ)\n"
    )
    rows, bad = [], 0
    for p in pages:
        rel = p.relative_to(ROOT).as_posix()
        print(f"{D}… {rel} (×{a.k}){OFF}", flush=True)
        verdict, detail, _ = review(p, k=a.k, workers=workers)
