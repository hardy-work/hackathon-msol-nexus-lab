#!/usr/bin/env python3
"""Long-lived, access-scoped Knowledge Base query engine."""
...
def suggested_actions(query: str, status: str, citations: list[str]) -> list[dict[str, object]]:
    ...
    return [{
        ...
        "description": "Chuyển yêu cầu ghi/cập nhật sang action skill có permission; Knowledge Base không tự thay đổi dữ liệu.",
...
class KnowledgeRuntime:
    ...
    def _token(self) -> str:
        interval = max(0.1, float(os.getenv("KNOWLEDGE_BASE_VERSION_CHECK_SECONDS", "5")))
        now = time.monotonic()
    ...
    def query(self, project: str, question: str, *, llm: bool = False,
        ...
        if not allowed:
            return finish({"status": "forbidden",
                           "answer": "Bạn không có quyền đọc Knowledge Base của dự án này.",
        ...
        try:
            ...
            return finish({"status": "error", "answer": "Không thể truy vấn knowledge base.",
                           "confidence": "none", "citations": [],
