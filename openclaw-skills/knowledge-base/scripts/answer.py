...
class KB:
    ...
    def signed(self, relation):
        ...
        if not c or not c[4] or not c[5] or not c[6] or str(c[3]).startswith("TODO"):
            return False
        try:
            grants = json.loads(os.getenv("KNOWLEDGE_BASE_COVERAGE_GRANTS", "{}"))
        except json.JSONDecodeError:
            return False
        approval_ids = {
            x.strip() for x in os.getenv("KNOWLEDGE_BASE_APPROVAL_IDS", "").split(",")
            if x.strip()
        }
...
def tier1(kb, q):
    ...
    if router.is_action_request(q):
        return Result(1, NF,
                      ...
                      reason="Knowledge Base chỉ tạo action proposal; action skill riêng phải kiểm tra quyền và xin approval trước khi ghi.")
