...
when ``KNOWLEDGE_BASE_LLM=1`` is set for the demo, this check must prove that
...
def main() -> int:
    ...
    env = os.environ.copy()
    env.update({
        "KNOWLEDGE_BASE_LLM": "1",
        "KNOWLEDGE_BASE_ACTOR": env.get("KNOWLEDGE_BASE_ACTOR", "local-demo"),
        "KNOWLEDGE_BASE_ROLES": env.get("KNOWLEDGE_BASE_ROLES", "project_member"),
        "KNOWLEDGE_BASE_DEMO_MODE": env.get("KNOWLEDGE_BASE_DEMO_MODE", "1"),
    })
    command = [
        sys.executable, str(ROOT / "scripts/run.py"),
        "--project", "nexus", "--query", QUESTION,
        "--actor", env["KNOWLEDGE_BASE_ACTOR"],
        "--roles", env["KNOWLEDGE_BASE_ROLES"],
        "--no-cache",
    ]
    try:
